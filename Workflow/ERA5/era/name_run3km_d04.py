...
dir_in = "/glade/scratch/srahimi/CMIP6/input4MIP/"
...
for iyear in range(year1,year2,inc):
 ...
 #Original file
 file_orig = "run3km_namelist_%s.input" %(domain)

 new_dir = "namelist.input_run3km_%s_files/" %(domain)
 new_file = "namelist.input_run3km_%s_%s" %(iyear,domain)
 fo = open(file_orig,"r")
 ...
 directive = "cp %s%s %s/namelist.input_run3km_%s" %(new_dir,new_file,dir_new,domain)
 os.system(directive)
